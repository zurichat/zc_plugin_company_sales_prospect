...
const EmailTemplate = () => {
  ...
  useEffect(()=>{
    customAxios.get(getEmailTemplateURL).then(
      res=>{
       console.log( 'backenddata',res.data.data);
  ...
  return (
    <EmailDataContext.Provider value={templates, setTemplates}>
      ...
      <div className="p-10 w-screen">
        ...
        <div className="my-12 flex flex-row bg-blue-500 w-full justify-around flex-wrap">
          {templates.map((an, i) => (
            <div
              key={i}
              className="w-80 hover-w-96 cursor-pointer my-4 mx-8"
              onClick={handleOpenSuccessModal}
            >
              <Link
              to = {{ pathname: '/template',   state : an   }}
              >
